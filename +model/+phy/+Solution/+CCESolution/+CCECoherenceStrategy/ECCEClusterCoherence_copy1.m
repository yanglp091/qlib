classdef ECCEClusterCoherence < model.phy.Solution.CCESolution.CCECoherenceStrategy.AbstractClusterCoherence
    ...
    methods
        ...
        function coh=calculate_cluster_coherence(obj,center_spin_states,timelist,varargin)
            ...
            dim=denseMat.dim;
            denseMat.setMatrix(speye(dim)/dim);
            
            if dim<1000
                coh=calculate_coherence_hilbert(bath_cluster,hami_list,hami_prefactor,denseMat,timelist);
            else
                coh=calculate_coherence_liouville(bath_cluster,hami_list,hami_prefactor,denseMat,timelist);
            end
            obj.coherence=coh;
        end
...
function coh=calculate_coherence_hilbert(bath_cluster,hami_list,hami_prefactor,denseMat,timelist)
    %Observable
    obs=model.phy.QuantumOperator.SpinOperator.Observable(bath_cluster,'IdentityMatrix');
    obs.setMatrix(1);
    % Evolution
    d_mat_evolution=model.phy.Dynamics.EvolutionKernel.DensityMatrixEvolution(hami_list,'MixedState',hami_prefactor);
...
function coh=calculate_coherence_liouville(bath_cluster,hami_list,hami_prefactor,denseMat,timelist)
    %Observable
    obs=model.phy.QuantumOperator.SpinOperator.Observable(bath_cluster,'IdentityMatrix');
    dim=obs.dim;
    obs.setMatrix(speye(dim));
    % Evolution
    [liouList,prefactor]=get_liouvillian_list(hami_list,hami_prefactor);
    d_mat_evolution=model.phy.Dynamics.EvolutionKernel.MatrixVectorEvolution(liouList,'MixedState',prefactor);
    dynamics=model.phy.Dynamics.QuantumDynamics(d_mat_evolution);
...
function [liouList,prefactor]=get_liouvillian_list(hami_list,hami_prefactor)
